...
##########################################################
# by cohortSpeciesRiver - model is overfit?

modCSR <- bam(observedLength ~ cohortCentered * species * river + 
               s(ydayCumul, bs = 'cr') +
...
predGridCSR <- expand.grid(cohortSpeciesRiver = unique(modCSR$model$cohortSpeciesRiver),
                           ydayCumul = seq(250,1000, by = 30)) %>%
                    mutate(cohortOrdered = as.numeric(substr(cohortSpeciesRiver,1,4)),
                           speciesOrdered = substr(cohortSpeciesRiver,6,8),
                           riverOrdered = substring(cohortSpeciesRiver,10),
...
ggplot(predGrid2CSR, aes(ydayCumul,pred, color = species)) +
  geom_point() + 
  geom_line() +
  ylim(50,210) +
...
modSR <- bam(observedLength ~ cohortCentered * species * river + 
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', by = speciesOrdered) +
               s(ydayCumul, bs = 'cr', by = riverOrdered) +
               s(ydayCumul, bs = 'cr', by = speciesRiverOrdered), 
...
# by speciesCohort
system.time(
modSC <- bam(observedLength ~ cohortCentered * species * river + 
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', by = speciesOrdered) +
               s(ydayCumul, bs = 'cr', by = speciesCohortOrdered), 
...
modCR <- bam(observedLength ~ cohortCentered * species * river + 
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', by = riverOrdered) +
               s(ydayCumul, bs = 'cr', by = cohortRiverOrdered), 
...
modCSRY <- bam(observedLength ~ cohortCentered * species * river + 
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', by = speciesOrdered) +
               s(ydayCumul, bs = 'cr', by = riverOrdered) +
...
mod <- bam(observedLength ~ cohortCentered * species * river + 
               s(ydayCumul, bs = 'cr'), 
               data = vB %>% filter(cohort %in% 2000:2013, species %in% c('bkt','bnt')), 
               method = "REML")
summary(modCSRY)
...
AIC(modCSRY,modCSR,modSR,modSC,modCR,mod)  %>% rownames_to_column() %>% arrange(AIC)
