...
vB$riverFactor <- factor(vB$river, levels = c('bkt','bnt'))
...
kIn <- 4
dataIn <- vB %>% filter(cohort %in% 2000:2013, species %in% c('bkt','bnt'))
```
...
modCSR <- bam(observedLength ~ cohort * species * river + 
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = speciesOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = speciesCohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = speciesRiverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortSpeciesRiverOrdered), 
             data = dataIn, method = "REML")
summary(modCSR)
#plot(modCSR)

predGridCSR <- expand.grid(cohortSpeciesRiverOrdered = unique(modCSR$model$cohortSpeciesRiverOrdered),
                           ydayCumul = seq(250,1000, by = 30)) %>%
                    mutate(cohortOrdered = as.numeric(substr(cohortSpeciesRiverOrdered,1,4)),
                           speciesOrdered = substr(cohortSpeciesRiverOrdered,6,8),
                           riverOrdered = substring(cohortSpeciesRiverOrdered,10),
                           cohortRiverOrdered = paste(cohortOrdered, riverOrdered, sep = '_'),
                           speciesCohortOrdered = paste(speciesOrdered, cohortOrdered, sep = '_'),
                           speciesRiverOrdered = paste(speciesOrdered, riverOrdered, sep = '_'),
                           cohortCentered = cohortOrdered - cohortOffset,
                           cohort = cohortOrdered,
                           species = speciesOrdered,
                           river = riverOrdered)

predCSR <- predict(modCSR, newdata = predGridCSR)#, type = 'lpmatrix')

predGrid2CSR <- bind_cols(predGridCSR,as.data.frame(predCSR))

ggplot(predGrid2CSR, aes(ydayCumul,predCSR, color = species)) +
  geom_point() + 
  geom_line() +
  ylim(50,210) +
...
##########################################################
# by cohortSpeciesRiver - additive splines, no 3-way spline
system.time(
modC_S_R <- bam(observedLength ~ cohort * species * river + 
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = speciesOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
...
predGridC_S_R <- expand.grid(cohortSpeciesRiverOrdered = unique(modCSR$model$cohortSpeciesRiverOrdered), # use modCSR to get 3-way
                               ydayCumul = seq(250,1000, by = 30)) %>%
                    mutate(cohortOrdered = as.numeric(substr(cohortSpeciesRiverOrdered,1,4)),
                           speciesOrdered = substr(cohortSpeciesRiverOrdered,6,8),
                           riverOrdered = substring(cohortSpeciesRiverOrdered,10),
...
predC_S_R <- predict(modC_S_R, newdata = predGridC_S_R)#, type = 'lpmatrix')

predGrid2C_S_R <- bind_cols(predGridC_S_R,as.data.frame(predC_S_R))

ggplot(predGrid2C_S_R, aes(ydayCumul,predC_S_R, color = species)) +
  geom_point() + 
  geom_line() +
  ylim(50,210) +
  facet_grid(river ~ cohort) +
...
modSR <- bam(observedLength ~ cohort * species * river + 
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = speciesOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = speciesRiverOrdered), 
...
predGridSR <- expand.grid(speciesRiverOrdered = unique(modSR$model$speciesRiverOrdered),
                          cohortOrdered = 2000:2013,
                          ydayCumul = seq(250,1000, by = 30)) %>%
                   mutate(speciesOrdered = substr(speciesRiverOrdered,1,3),
                          riverOrdered = substring(speciesRiverOrdered,5),
                          cohortCentered = cohortOrdered - cohortOffset,
                          cohort = cohortOrdered,
                          species = speciesOrdered,
                          river = riverOrdered)

predSR <- predict(modSR, newdata = predGridSR)#, type = 'lpmatrix')

predGrid2SR <- bind_cols(predGridSR,as.data.frame(predSR))

ggplot(predGrid2SR, aes(ydayCumul,predSR, color = species)) +
  geom_point() + 
  geom_line() +
  facet_grid(river ~ cohortOrdered)+
  ggtitle('modSR')

#######################################
# by speciesCohort
system.time(
modSC <- bam(observedLength ~ cohort * species * river + 
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = speciesOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = speciesCohortOrdered), 
...
predGridSC <- expand.grid(speciesCohortOrdered = unique(modSC$model$speciesCohortOrdered),
                          river = unique(vB$riverOrdered),
                          ydayCumul = seq(250,1000, by = 30)) %>%
                   mutate(speciesOrdered = substr(speciesCohortOrdered,1,3),
                          cohortOrdered = as.numeric(substring(speciesCohortOrdered,5)),
                          cohortCentered = cohortOrdered - cohortOffset,
                          cohort = cohortOrdered,
                          species = speciesOrdered)

predSC <- predict(modSC, newdata = predGridSC)#, type = 'lpmatrix')

predGrid2SC <- bind_cols(predGridSC,as.data.frame(predSC))

ggplot(predGrid2SC, aes(ydayCumul,predSC, color = species)) +
  geom_point() + 
  geom_line() +
  facet_grid(river ~ cohortOrdered) +
  ggtitle('modSC')
...
modCR <- bam(observedLength ~ cohort * species * river + 
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered), 
...
ggplot(predGrid2CR, aes(ydayCumul, predCR, color = species)) +
  geom_point() + 
  geom_line() +
  ylim(50,200) +
  facet_grid(river ~ cohortOrdered) +
...
# add year to smooths, model seems overfit

# try running without 'speciesOrdered' for year  this does a difference from the main fit, i.e. try s(year, bs = 'cr', by = speciesFactor)

modCSRY <- bam(observedLength ~ cohort * species * river + 
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = speciesOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
               ...
               s(ydayCumul, bs = 'cr', k = kIn, by = speciesRiverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortSpeciesRiverOrdered) +
               #s(year, bs = 'cr', by = speciesOrdered), 
               s(year, bs = 'cr', k = kIn, by = speciesFactor),
               data = dataIn, 
               method = "REML")
summary(modCSRY)
plot(modCSRY, pages=10, scale = 0)

predGridCSRY <- expand.grid(cohortSpeciesRiverOrdered = unique(modCSRY$model$cohortSpeciesRiverOrdered),
                            year = unique(modCSRY$model$year),
                            ydayCumul = seq(250,1000, by = 30)) %>%
                    mutate(cohortOrdered = as.numeric(substr(cohortSpeciesRiverOrdered,1,4)),
                           speciesOrdered = substr(cohortSpeciesRiverOrdered,6,8),
                           riverOrdered = substring(cohortSpeciesRiverOrdered,10),
...
predCSRY <- predict(modCSRY, newdata = predGridCSRY)#, type = 'lpmatrix')

predGrid2CSRY <- bind_cols(predGridCSRY,as.data.frame(predCSRY))

ggplot(predGrid2CSRY, aes(ydayCumul, predCSRY, color = species)) +
  geom_point() + 
  geom_line() +
  ylim(50,300) +
  facet_grid(river ~ cohortOrdered) +
...
# trying a couple of models
modCSRC <- bam(observedLength ~ cohort * species * river + 
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = speciesOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
               ...
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortSpeciesRiverOrdered) +
               #s(year, bs = 'cr', by = speciesOrdered), 
               s(cohort, bs = 'cr', k = kIn, by = speciesFactor),
               data = dataIn, 
               method = "REML")
summary(modCSRC)
plot(modCSRC, pages=8, scale = 0)

modC_S_RC <- bam(observedLength ~ cohort + species + river + 
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = speciesOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
...
# modCSR_Ind <- bam(observedLength ~ cohort * species * river + 
#                s(ydayCumul, bs = 'cr') +
#                s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
#                s(ydayCumul, bs = 'cr', k = kIn, by = speciesOrdered) +
#                s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
...
mod <- bam(observedLength ~ cohort * species * river + 
               s(ydayCumul, bs = 'cr'), 
               data = dataIn, 
               method = "REML")
summary(mod)

predGrid <- expand.grid(cohortSpeciesRiverOrdered = unique(modCSR$model$cohortSpeciesRiverOrdered),
                        ydayCumul = seq(250,1000, by = 30)) %>%
                    mutate(cohortOrdered = as.numeric(substr(cohortSpeciesRiverOrdered,1,4)),
                           speciesOrdered = substr(cohortSpeciesRiverOrdered,6,8),
                           riverOrdered = substring(cohortSpeciesRiverOrdered,10),
...
save(mod,modCR,modCSR,modSC,modSR,modCSRY,modC_S_R, file = './data/out/mods.RData')
AIC(modC_S_RC,modC_S_R,modCSR,modSR,modSC,modCR,mod)  %>% rownames_to_column() %>% arrange(AIC)
