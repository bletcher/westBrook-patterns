...
ggplot(vB %>% filter(cohort %in% 2000:2013, species %in% c('bkt', 'bnt', 'ats')), aes(ydayCumul,cf, group = species, color = (species))) +
  geom_point(size = 0.5, alpha = 0.1) +
 # geom_smooth(span = 0.1, size = 1.5, se = F) +
  geom_smooth(method='gam', alpha=0.1, formula=y~s(x, k = kValue), se = F) +
...
vB$yearOrdered <- factor(vB$year, levels = 2000:2013)
vB$yearOrderedSpecies <- factor(paste(vB$yearOrdered,vB$species, sep = '_'))

# 1. Do we need a RE on year in s()? YES, yearOrdered is best
# 
mod0_0 <- gam(observedLength ~ year + s(ydayCumul, bs = 'cr'), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")

mod0_1 <- gam(observedLength ~ year + s(ydayCumul, bs = 'cr', by = (year)), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")

mod0_2 <- gam(observedLength ~ year + s(ydayCumul, bs = 'cr', by = (yearFactor)), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")

mod0_3 <- gam(observedLength ~ year + s(ydayCumul, bs = 'cr', by = (yearOrdered)), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")

mod0_4 <- gam(observedLength ~ year + s(ydayCumul, bs = 'cr', by = (yearOrderedSpecies)), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")
...
mod0_3a <- gam(observedLength ~ year + s(ydayCumul, bs = 'cr') + s(ydayCumul, bs = 'cr', by = (yearOrdered)), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")

mod0_3b <- gam(observedLength ~ yearOrdered + s(ydayCumul, bs = 'cr') + s(ydayCumul, bs = 'cr', by = (yearOrdered)), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")

# mod0_3b is best, but we want to estimate a trend as in mod0_3a
# add + s(ydayCumul, bs = 'cr') to models
AIC(mod0_3,mod0_3a,mod0_3b) %>% rownames_to_column() %>% arrange(AIC)


# 3. gam vs. bam for this big dataset? bam() is MUCH faster!
system.time( mod0_3a_1 <- gam(observedLength ~ year + s(ydayCumul, bs = 'cr') + s(ydayCumul, bs = 'cr', by = (yearOrdered)), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML") )
# 316 sec
summary(mod0_3a_1)

system.time( mod0_3a_2 <- bam(observedLength ~ year + s(ydayCumul, bs = 'cr') + s(ydayCumul, bs = 'cr', by = (yearOrdered)), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML") )
# 8 sec
summary(mod0_3a_2)

# 4. bs = 'cr', vs default 'tp'. AIC for 'cr' is ~ 300 lower. Stick with 'cr'
mod0_3a_2a <- bam(observedLength ~ year + s(ydayCumul) + s(ydayCumul, by = (yearOrdered)), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")

AIC(mod0_3a_2,mod0_3a_2a)
...
mod0 <- bam(observedLength ~ year + s(ydayCumul, bs = 'cr') + s(ydayCumul, bs = 'cr', by = yearOrdered), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")

mod1 <- bam(observedLength ~ species + s(ydayCumul, bs = 'cr') + s(ydayCumul, bs = 'cr', by = yearOrdered), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")

mod2 <- bam(observedLength ~ river + s(ydayCumul, bs = 'cr') + s(ydayCumul, bs = 'cr', by = yearOrdered), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")

# river expains the most for 1-variable models
AIC(mod0,mod1,mod2) %>% rownames_to_column() %>% arrange(AIC)

# 2-way models
mod3 <- bam(observedLength ~ year * species + s(ydayCumul, bs = 'cr') + s(ydayCumul, bs = 'cr', by = yearOrdered), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")

mod4 <- bam(observedLength ~ year * river + s(ydayCumul, bs = 'cr') + s(ydayCumul, bs = 'cr', by = yearOrdered), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")

mod5 <- bam(observedLength ~ species * river + s(ydayCumul, bs = 'cr') + s(ydayCumul, bs = 'cr', by = yearOrdered), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")

# mod4 is best
AIC(mod0,mod1,mod2,mod3,mod4,mod5) %>% rownames_to_column() %>% arrange(AIC)

# 3-way models
mod6 <- bam(observedLength ~ year * species * river + s(ydayCumul, bs = 'cr') + s(ydayCumul, bs = 'cr', by = yearOrdered), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")
summary(mod6)

# Any additive 3-way better? NO
mod7 <- bam(observedLength ~ year + species * river + s(ydayCumul, bs = 'cr') + s(ydayCumul, bs = 'cr', by = yearOrdered), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')))

mod8 <- bam(observedLength ~ year * species + river + s(ydayCumul, bs = 'cr') + s(ydayCumul, bs = 'cr', by = yearOrdered), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')))

mod9 <- bam(observedLength ~ year + species + river + s(ydayCumul, bs = 'cr') + s(ydayCumul, bs = 'cr', by = yearOrdered), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')))
