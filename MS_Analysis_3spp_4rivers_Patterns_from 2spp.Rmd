...
modCSR <- bam(observedLength ~ cohort * species * river + 
               ...
               s(ydayCumul, bs = 'cr', by = speciesOrdered) +
               s(ydayCumul, bs = 'cr', by = riverOrdered) +
               s(ydayCumul, bs = 'cr', by = cohortRiverOrdered) +
               s(ydayCumul, bs = 'cr', by = speciesCohortOrdered) +
               s(ydayCumul, bs = 'cr', by = speciesRiverOrdered) +
               s(ydayCumul, bs = 'cr', by = cohortSpeciesRiverOrdered), 
...
##########################################################
# by cohortSpeciesRiver - additive splines
system.time(
modCSR_add <- bam(observedLength ~ cohort * species * river + 
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', by = speciesOrdered) +
               s(ydayCumul, bs = 'cr', by = riverOrdered) +
...
predGridCSR_add <- expand.grid(cohortSpeciesRiverOrdered = unique(modCSR_add$model$cohortSpeciesRiverOrdered),
                           ydayCumul = seq(250,1000, by = 30)) %>%
                    mutate(cohortOrdered = as.numeric(substr(cohortSpeciesRiverOrdered,1,4)),
                           speciesOrdered = substr(cohortSpeciesRiverOrdered,6,8),
                           riverOrdered = substring(cohortSpeciesRiverOrdered,10),
...
predCSR_add <- predict(modCSR_add, newdata = predGridCSR_add)#, type = 'lpmatrix')

predGrid2CSR_add <- bind_cols(predGridCSR_add,as.data.frame(predCSR_add))

ggplot(predGrid2CSR_add, aes(ydayCumul,predCSR_add, color = species)) +
  geom_point() + 
  geom_line() +
  ylim(50,210) +
  facet_grid(river ~ cohort)
...
save(mod,modCR,modCSR,modSC,modSR,modCSRY, file = './data/out/mods.RData')
AIC(modCSR,modSR,modSC,modCR,mod)  %>% rownames_to_column() %>% arrange(AIC)
