...
# 1. Do we need a RE on year in s()? YES
mod0a <- gam(observedLength ~ year + s(ydayCumul, bs = 'cr'), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")
summary(mod1)

mod0 <- gam(observedLength ~ year + s(ydayCumul, bs = 'cr', by = (year)), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")
summary(mod1)

AIC(mod0,mod0a) %>% rownames_to_column() %>% arrange(AIC)

# 2. Best structure for year,species,river
mod1 <- gam(observedLength ~ species + s(ydayCumul, bs = 'cr', by = (year)), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")

mod2 <- gam(observedLength ~ river + s(ydayCumul, bs = 'cr', by = (year)), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")

# river expains the most for 1-variable models
AIC(mod0,mod1,mod2) %>% rownames_to_column() %>% arrange(AIC)

# 2-way models
mod3 <- gam(observedLength ~ year * species + s(ydayCumul, bs = 'cr', by = (year)), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")

mod4 <- gam(observedLength ~ year * riverOrdered * riverOrdered + s(ydayCumul, bs = 'cr', by = (year)), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")
summary(mod4)

mod5 <- gam(observedLength ~ species * riverOrdered * riverOrdered + s(ydayCumul, bs = 'cr', by = (year)), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")
summary(mod4)
...
# 3-way models
mod6 <- gam(observedLength ~ year * species * riverOrdered + s(ydayCumul, bs = 'cr', by = (year)), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")

# https://www.fromthebottomoftheheap.net/2017/12/14/difference-splines-ii/
# calculates differences from overall spline for each year
# a lot slower
vB$yearOrdered <- factor(vB$year, levels = 2000:2013)
mod6a <- gam(observedLength ~ year * species * riverOrdered + s(ydayCumul, bs = 'cr', by = (yearOrdered)), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")

# try with river instead of riverOrdered
mod6b <- gam(observedLength ~ year * species * river + s(ydayCumul, bs = 'cr', by = (yearOrdered)), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')), method = "REML")
summary(mod6b)


# 3-way is best
AIC(mod1,mod2,mod3,mod4,mod5,mod6,mod6a) %>% rownames_to_column() %>% arrange(AIC)

# Any additive 3-way better? NO
mod7 <- gam(observedLength ~ year + species * riverOrdered + s(ydayCumul, bs = 'cr', by = (year)), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')))

mod8 <- gam(observedLength ~ year * species + riverOrdered + s(ydayCumul, bs = 'cr', by = (year)), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')))

mod9 <- gam(observedLength ~ year + species + riverOrdered + s(ydayCumul, bs = 'cr', by = (year)), data = vB %>% filter(cohort %in% 2000:2013,species %in% c('bkt','bnt')))
