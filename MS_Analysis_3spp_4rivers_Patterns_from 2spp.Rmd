...
modCSR_C <- bam(observedLength ~ cohort * species * river + 
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = speciesOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
               ...
               method = "REML")
summary(modCSR_C)
plot(modCSR_C, pages=10, scale = 0)
...
save(mod,modCR,modCSR,modSC,modSR,modC_S_R,modCSR_C, file = './data/out/mods.RData')
AIC(modCSR_C,modC_S_R,modCSR,modSR,modSC,modCR,mod)  %>% rownames_to_column() %>% arrange(AIC)
