...
# only including river, season, salmon01 becaseu we assume these have categorical effects

mod0 <- bam(observedLength ~ riverFactor * factor(season) + factor(salmon01) + 
               s(ydayCumul, bs = 'cr'), 
               data = dataIn, 
               method = "REML")

mod1 <- bam(observedLength ~ riverFactor + factor(season) + factor(salmon01) + 
               s(ydayCumul, bs = 'cr'), 
               data = dataIn, 
               method = "REML")
...
mod3 <- bam(observedLength ~ riverFactor + factor(season) + 
               s(ydayCumul, bs = 'cr'), 
               data = dataIn, 
               method = "REML")
...
mod5 <- bam(observedLength ~ factor(season) + 
               s(ydayCumul, bs = 'cr'), 
               data = dataIn, 
               method = "REML")
...
AIC(mod0,mod1,mod2,mod3,mod4,mod5,mod6) %>% rownames_to_column() %>% arrange(AIC)
summary(mod0)
plot(mod0)
gam.check(mod0)
...
```{r gam structure given best model from above}
...
system.time(
mod0_YDCcr_C_CSr_Tr_Fr_TFr <- bam(observedLength ~ riverFactor * factor(season) + factor(salmon01) +              
               s(ydayCumul, bs = 'cr') +
               ...
               s(cohort, bs = 'cr', k = kIn) + 
               s(cohortStrengthScaled, bs = 'cr', k = kIn, by = riverOrdered) +  # this is conspecific cS only
               s(meanTemperatureScaledBySeason, bs = 'cr', k = kIn, by = riverOrdered) +
               s(meanFlowScaledBySeason, bs = 'cr', k = kIn, by = riverOrdered) +
               s(meanTemperatureFlowScaledBySeason, bs = 'cr', k = kIn, by = riverOrdered), 
               data = dataIn, 
               method = "REML")
)
...
system.time(
mod0_CR_Cr_CS_Tr_Fr_TFr <- bam(observedLength ~ riverFactor * factor(season) + factor(salmon01) +              
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered) +
...
mod0_CR_Cr_CS_Tr_Fr_TFr <- bam(observedLength ~ riverFactor * factor(season) + factor(salmon01) +              
               ...
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered) +
               s(cohort, bs = 'cr', k = kIn, by = riverOrdered) + 
               s(cohortStrengthScaled, bs = 'cr', k = kIn) +  # this is conspecific cS only
               s(meanTemperatureScaledBySeason, bs = 'cr', k = kIn, by = riverOrdered) +
               s(meanFlowScaledBySeason, bs = 'cr', k = kIn, by = riverOrdered) +
               s(meanTemperatureFlowScaledBySeason, bs = 'cr', k = kIn, by = riverOrdered), 
...
system.time(
mod0_CR_Cr_CSr_T_F_TF <- bam(observedLength ~ riverFactor * factor(season) + factor(salmon01) +              
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered) +
               ...
               s(meanTemperatureFlowScaledBySeason, bs = 'cr'), 
               data = dataIn, 
               method = "REML")
)
...
system.time(
mod0_CR_Cr_CSr <- bam(observedLength ~ cohort * factor(river) * factor(salmon01) + meanTemperatureScaledBySeason * meanFlowScaledBySeason * factor(season) + cohortStrengthScaled +              
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered) +
...
system.time(
mod0_CR_Cr_Tr_Fr_TFr <- bam(observedLength ~ cohort * factor(river) * factor(salmon01) + meanTemperatureScaledBySeason * meanFlowScaledBySeason * factor(season) + cohortStrengthScaled +              
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered) +
...
system.time(
mod0_CR_CSr_Tr_Fr_TFr <- bam(observedLength ~ cohort * factor(river) * factor(salmon01) + meanTemperatureScaledBySeason * meanFlowScaledBySeason * factor(season) + cohortStrengthScaled +              
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered) +
...
system.time(
mod0_CSr_Tr_Fr_TFr <- bam(observedLength ~ cohort * factor(river) * factor(salmon01) + meanTemperatureScaledBySeason * meanFlowScaledBySeason * factor(season) + cohortStrengthScaled +              
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered) +
...
system.time(
mod0_CR_Cr_CS_T_F_TF <- bam(observedLength ~ cohort * factor(river) * factor(salmon01) + meanTemperatureScaledBySeason * meanFlowScaledBySeason * factor(season) + cohortStrengthScaled +              
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered) +
...
system.time(
mod0_CR_Cr_CS_Tr_F_TF <- bam(observedLength ~ cohort * factor(river) * factor(salmon01) + meanTemperatureScaledBySeason * meanFlowScaledBySeason * factor(season) + cohortStrengthScaled +              
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered) +
...
system.time(
mod0_C_Cr_CS_Tr_F_TF <- bam(observedLength ~ cohort * factor(river) * factor(salmon01) + meanTemperatureScaledBySeason * meanFlowScaledBySeason * factor(season) + cohortStrengthScaled +              
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered) +
               s(cohort, bs = 'cr', k = kIn, by = riverOrdered) + 
...
####
system.time(
mod0_C_Cr_CS_T_F_TF <- bam(observedLength ~ cohort * factor(river) * factor(salmon01) + meanTemperatureScaledBySeason * meanFlowScaledBySeason * factor(season) + cohortStrengthScaled +              
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
              # s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered) +
...
tmp <- 
bam(observedLength ~ factor(river) * factor(season) + factor(salmon01) + 
      #factor(river) *  + meanTemperatureScaledBySeason * meanFlowScaledBySeason * factor(season) + cohortStrengthScaled +              
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
              # s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered) +
               ...
               method = "REML")
summary(tmp)
plot(tmp, scale = 0, shade = TRUE)


AIC(mod0, mod0_CR, mod0_CR_Cr_CSr_Tr_Fr_TFr,mod0_CR_C_CSr_Tr_Fr_TFr,mod0_CR_Cr_CS_Tr_Fr_TFr,mod0_CR_Cr_CSr_T_F_TF,
    mod0_CR_Cr_CSr,mod0_CR_Cr_Tr_Fr_TFr,mod0_CR_CSr_Tr_Fr_TFr,mod0_CSr_Tr_Fr_TFr,mod0_CR_Cr_CS_T_F_TF,mod0_CR_Cr_CS_Tr_F_TF,mod0_C_Cr_CS_Tr_F_TF,mod0_C_Cr_CS_T_F_TF) %>% rownames_to_column() %>% arrange(AIC)

summary(mod0_CR_Cr_CS_T_F_TF)
gam.check(mod0_CR_Cr_CS_T_F_TF)
plot(mod0_CR_Cr_CS_T_F_TF, scale = 0, pages=8)
...
system.time(
modCR_SR_c_cSMP <- bam(observedLength ~ cohort * river + factor(salmon01) * river + metaPop01 +
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn,  by = riverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered) +
               s(cohort, bs = 'cr', k = kIn) + 
               s(cohortStrengthScaled, bs = 'cr', k = kIn, by = metaPop01),  # this is conspecific cS only
             data = dataIn, method = "REML")
)
summary(modCR_SR_c_cSMP)
...
system.time(
modCR_SR_c_cSR3b <- bam(observedLength ~ cohort * river + factor(salmon01) * river + metaPop01 +
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn,  by = riverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered) +
...
# take out salmon because the salmon01 is in cohortStrengthScaledATS
system.time(
modCR__c_cSR3b <- bam(observedLength ~ cohort * river + metaPop01 +
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn,  by = riverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered) +
...
AIC(mod,modC,modR,modC_R,modCR, modCR_c, modCR_S_c, modCR_c_cS, modCR_S_c_cS, modCR_SR_c_cS, modCR_SR_c_cSR, modCR_SR_c_cSMP, modCR_SR_c_cSR3, modCR_SR_c_cSR3a,modCR_SR_c_cSR3b, modCR__c_cSR3b) %>% rownames_to_column() %>% arrange(AIC)

save(mod,modC,modR,modC_R,modCR,modCR_c, modCR_S_c, modCR_c_cS, modCR_S_c_cS, modCR_SR_c_cS, modCR_SR_c_cSR, modCR_SR_c_cSMP, modCR_SR_c_cSR3, modCR_SR_c_cSR3a,modCR_SR_c_cSR3b, file = paste0('./data/out/mods_',speciesIn,'.RData'))
...
modCR_SR_c_cS_T <- bam(observedLength ~ cohort * river + factor(salmon01) * river +
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered) +
               s(cohort, bs = 'cr', k = kIn) + 
               s(cohortStrengthScaled, bs = 'cr', k = kIn) +  # this is conspecific cS only
               s(meanTemperatureScaled, bs = 'cr', k = kIn),
             data = dataIn, method = "REML")

modCR_SR_c_cS_F <- bam(observedLength ~ cohort * river + factor(salmon01) * river +
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered) +
               s(cohort, bs = 'cr', k = kIn) + 
               s(cohortStrengthScaled, bs = 'cr', k = kIn) +  # this is conspecific cS only
               s(meanFlowScaled, bs = 'cr', k = kIn),
             data = dataIn, method = "REML")

modCR_SR_c_cS_T_F <- bam(observedLength ~ cohort * river + factor(salmon01) * river +
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered) +
...
modCR_SR_c_cS_TF <- bam(observedLength ~ cohort * riverOrdered + factor(salmon01) * riverOrdered +
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered) +
               ...
               s(meanTemperatureScaled, bs = 'cr', k = kIn) +
               s(meanFlowScaled, bs = 'cr', k = kIn) +
               s(meanTemperatureFlowScaled, bs = 'cr', k = kIn),
             data = dataIn, method = "REML")


modCR_SR_c_cS_TFs <- bam(observedLength ~ cohort * river + factor(salmon01) * river + meanTemperatureScaled * meanFlowScaled +
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered) +
               s(cohort, bs = 'cr', k = kIn) + 
               s(cohortStrengthScaled, bs = 'cr', k = kIn) +  # this is conspecific cS only
               s(meanTemperatureScaled, bs = 'cr', k = kIn, by = factor(season)) +
               s(meanFlowScaled, bs = 'cr', k = kIn, by = factor(season)) +
               s(meanTemperatureFlowScaled, bs = 'cr', k = kIn, by = factor(season)),
             data = dataIn, method = "REML")


summary(modCR_SR_c_cS_TFs)
plot(modCR_SR_c_cS_TFs, scale = 0, pages=8)

modCR_SR_c_cS_TFs2 <- bam(observedLength ~ cohort * river + factor(salmon01) * river + meanTemperatureScaled * meanFlowScaled * factor(season) +
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered) +
...
modCR_SR_c_cS_TFr <- bam(observedLength ~ cohort * river + factor(salmon01) * river +
               s(ydayCumul, bs = 'cr') +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = riverOrdered) +
               s(ydayCumul, bs = 'cr', k = kIn, by = cohortRiverOrdered) +
               s(cohort, bs = 'cr', k = kIn) + 
               s(cohortStrengthScaled, bs = 'cr', k = kIn) +  # this is conspecific cS only
               s(meanTemperatureScaled, bs = 'cr', k = kIn, by = riverOrdered) +
               s(meanFlowScaled, bs = 'cr', k = kIn, by = riverOrdered) +
               s(meanTemperatureFlowScaled, bs = 'cr', k = kIn, by = riverOrdered),
             data = dataIn, method = "REML")

AIC(modCR_SR_c_cS,modCR_SR_c_cS_T, modCR_SR_c_cS_F, modCR_SR_c_cS_T_F, modCR_SR_c_cS_TF, modCR_SR_c_cS_TFs, modCR_SR_c_cS_TFs2, modCR_SR_c_cS_TFr) %>% rownames_to_column() %>% arrange(AIC)

summary(modCR_SR_c_cS_TFr)
plot(modCR_SR_c_cS_TFr, scale = 0, pages=8)


predGridTF <- expand.grid(cohortRiverOrdered = unique(modBKT$model$cohortRiverOrdered),
                           ydayCumul = seq(250,1000, by = 30),
                           salmon01 = c(0,1),
                           cohortStrengthScaledBKT = seq(-2,2, by = 0.1)) %>%
                    mutate(cohortOrdered = as.numeric(substr(cohortRiverOrdered,1,4)),
                           riverOrdered = substring(cohortRiverOrdered,6),
                           cohortRiverOrdered = paste(cohortOrdered, riverOrdered, sep = '_'),
                           cohortCentered = cohortOrdered - cohortOffset,
...
predTF <- predict(modCR_SR_c_cS_TFr, newdata = predGridTF)#, type = 'lpmatrix')
...
predGridBNT <- expand.grid(cohortRiverOrdered = unique(modBNT$model$cohortRiverOrdered),
                           ...
                           salmon01 = c(0,1),
                           metaPop01 = c(0,1),
                           cohortStrengthScaledBKT = seq(-2,2, by = 0.25),
                           cohortStrengthScaledBNT = seq(-2,2, by = 0.25),
                           cohortStrengthScaledATS = seq(-2,2, by = 0.25)) %>%
                    mutate(cohortOrdered = as.numeric(substr(cohortRiverOrdered,1,4)),
                           riverOrdered = substring(cohortRiverOrdered,6),
                           cohortRiverOrdered = paste(cohortOrdered, riverOrdered, sep = '_'),
                           cohortCentered = cohortOrdered - cohortOffset,
