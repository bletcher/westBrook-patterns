...
predGridBNT <- expand.grid(riverFactor = unique(dataIn$riverFactor), 
                           cohortFactor = unique(dataIn$cohort),
                           ydayCumul = seq(250,1000, by = 30),
                           salmon01 = c(0,1),
                           cohortStrengthScaledBKT = seq(-2.5,2.5, by = 0.5),
                           cohortStrengthScaledBNT = seq(-2.5,2.5, by = 0.5),
                           cohortStrengthScaledATS = seq(-2.5,2.5, by = 0.5)) %>%
                    #left_join(., yDayAll) %>%
                    mutate(cohortRiverFactor = paste(cohortFactor, riverFactor, sep = '_'),
                           cohort = cohortFactor)
