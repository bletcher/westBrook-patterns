...
ggplot(cohortStrengthWide %>% filter(cohort >= 2002), aes(cohortStrengthScaledBKT, cohortStrengthScaledBNT, color=factor(cohort))) +
  geom_point() +
...
ggplot(predGrid2Both %>% 
       ...
       aes(ydayCumul, pred, group = speciesGG, color = speciesGG)) +
  #geom_point() + 
  geom_vline(xintercept = c(365,365*2), color = 'grey60', linetype = 2) +
  geom_line(size = 1.1) +
  labs(y = 'Predicted body size (mm)',
...
ggsave('./figures/ydayCumulAcrossRiverBySpp.png', width = 11, height = 8, units='in')
...
ggplot(predGrid2Both %>% 
  ...
  theme(legend.position = "right", 
        legend.direction = "vertical",
        legend.title = element_text(face = "plain")) +
  facet_grid(rows = vars(speciesGG)) 

ggsave('./figures/ydayCumulAcrossSppByRiver.png', width = 8, height = 8, units='in')
