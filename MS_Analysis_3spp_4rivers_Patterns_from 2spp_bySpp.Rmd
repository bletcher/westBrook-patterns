...
# predGridBKT2 <- expand.grid(riverFactor = unique(dataIn$riverFactor), 
#                            
#                            ydayCumul = seq(250,1000, by = 10),
#                            salmon01 = c(0,1)) %>%
#                     left_join(., yDayAll) %>%
...
pred <- predict(mod6_YDCcr_Cr_CSr_Tr_Fr_TFr, newdata = predGrid)#, type = 'lpmatrix')
#predBKT2 <- predict(mod1, newdata = predGridBKT2)#, type = 'lpmatrix')

predGrid2 <- bind_cols(predGrid, as.data.frame(pred)) %>% mutate(pred = pred)
#predGrid2BKT2 <- bind_cols(predGridBKT2, as.data.frame(predBKT2)) %>% mutate(pred = predBKT2, species = 'bkt')
