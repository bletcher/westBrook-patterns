...
ggplot(cohortStrength %>% filter(cohort > 1999), aes(cohort, cohortStrengthScaled, group = speciesGG, shape = speciesGG, color = speciesGG)) +
  geom_point(size = 3) +
  geom_line(size = 1.2) +
  labs(x = 'Cohort',
       y = 'Cohort strength',
...
#sppoke with keith. decided focus on cohrot is less interesting than river effects and density effects
...
AIC(mod1,mod2,mod3,mod4) %>% rownames_to_column() %>% arrange(AIC)
```
