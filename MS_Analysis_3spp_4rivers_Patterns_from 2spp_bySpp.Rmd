...
AIC(mod0_YDCcr, mod0_YDCc_r, mod0_YDCcr_Cr_CSr_Tr_Fr_TFr, mod0_YDCcr_Cr_CSr_Ts_Fs_TFs,mod0_YDCcr_Cr_CSr_Tsr_Fsr_TFsr,mod0_YDCcr_C_CSr_Tsr_Fsr_TFsr,mod0_YDCcr_Cr_CS_Tsr_Fsr_TFsr,mod0_YDCcr_Cr_CSr,mod0_YDCcr_C_CS_Tsr_Fsr_TFsr,mod0_YDCcr_Cr_Tsr_Fsr_TFsr,mod0_YDCcr_CSr_Tsr_Fsr_TFsr, mod0_YDCcr_Tsr_Fsr_TFsr) %>% rownames_to_column() %>% arrange(AIC)
...
predGridBKT <- expand.grid(cohortRiverFactor = unique(mod0_YDCcr_Cr_CSr_Tsr_Fsr_TFsr$model$cohortRiverFactor),
                           seasonRiverFactor = unique(mod0_YDCcr_Cr_CSr_Tsr_Fsr_TFsr$model$seasonRiverFactor),
                           ydayCumul = seq(250,1000, by = 30),
                           salmon01 = c(0,1),
                           cohortStrengthScaled = 0, #seq(-1.5,1.5, by = 0.5),
                           meanTemperatureScaledBySeason = 0, #seq(-1.5,1.5, by = 0.5),
                           meanFlowScaledBySeason = 0) %>% #seq(-1.5,1.5, by = 0.5)) %>%
                          # meanTemperatureFlowScaledBySeason = seq(-1.5,1.5, by = 0.5)) %>%
                    mutate(cohortFactor = as.numeric(substr(cohortRiverFactor,1,4)),
                           riverFactor = substring(cohortRiverFactor,6),
                           season = substring(seasonRiverFactor,1),
                           #cohortRiverFactor = paste(cohortFactor, riverFactor, sep = '_'),
                           #seasonRiverFactor = paste(season, riverFactor, sep = '_'),
                           #cohortCentered = cohortOrdered - cohortOffset,
                           cohort = cohortFactor,
                           #river = riverFactor,
                           meanTemperatureFlowScaledBySeason = meanTemperatureScaledBySeason * meanFlowScaledBySeason)
...
yDayAll <- data.frame(y = 250:1000)
# pick up here
#### need to do something like this  left_join(yDayAll, ydayCumulSeason, by = c('yDay' = 'minY'))

predGridBKT <- expand.grid(riverFactor = unique(dataIn$riverFactor), 
                           cohortFactor = unique(dataIn$cohort),
                           ydayCumul = seq(250,1000, by = 30),
                           salmon01 = c(0,1),
                           cohortStrengthScaled = 0, #seq(-1.5,1.5, by = 0.5),
                           meanTemperatureScaledBySeason = 0, #seq(-1.5,1.5, by = 0.5),
                           meanFlowScaledBySeason = 0) %>%
                    left_join(., ydayCumulSeason) %>%
...
predBKT <- predict(mod0_YDCcr_Cr_CSr_Tsr_Fsr_TFsr, newdata = predGridBKT)#, type = 'lpmatrix')

predGrid2BKT <- bind_cols(predGridBKT, as.data.frame(predBKT)) %>% mutate(pred = predBKT, species = 'bkt')


ggplot(predGrid2BKT %>% filter(salmon01 == 0),
...
#  guide_legend(title = 'Cohort') +
  facet_grid(~riverFactor, scales = 'free_y') 
